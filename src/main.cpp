...
{
	s << "Dimensions: (" << topography.xNumber() << ", " << topography.yNumber()
			<< ")" << endl;
	s << "Increment:  (" << topography.xIncrement() << ", "
			<< topography.yIncrement() << ")" << endl;
	s << "start pos:  (" << topography.startX() << ", " << topography.startY()
			<< ")" << endl;
...
std::string getReferenceTime( const wdb::GdalLoadConfiguration &conf,
		const std::vector<boost::filesystem::path> &files, WDB_LOG &log )
{
	...
	if( !conf.loading().referenceTime.empty() ){
		...
		ot << d.year() << "-" << d.month() << "-" << d.day() << " " << t.hours()
				<< ":" << t.minutes() << ":" << t.seconds() << "+00";

		log.infoStream() << "Reference time is based on mtime to the file '"
				<< files.front() << "'. Reference time: " << ot.str() << ".";
...
void test( const std::vector<boost::filesystem::path> &files )
{
	...
	cerr << "Elevation 'Mount Everest': ( 86.922623, 27.986065 ): " << h << endl
			<< endl;

	h = gdal.elevation( 8.309198, 61.637023 ); //elevation 2296m
	cerr << "Elevation 'Galdhøpiggen': ( 8.309198, 61.637023 ): " << h << endl
			<< endl;

	h = gdal.elevation( 88.147535, 27.7024914 ); //8586
	cerr << "Elevation 'Kanchenjunga': ( 88.146667, 27.7025 ): " << h << endl
			<< endl;

	h = gdal.elevation( 19.14453506, 69.60725813 ); //elevation > ~ 1072m
	cerr << "Elevation 'Tromsdalstinden': (  19.14453506, 69.60725813 ): " << h
			<< endl << endl;

	h = gdal.elevation( 10.708382, 59.924696 ); //elevation > ~ 47m
	cerr << "Elevation 'Vigelands parken': ( 10.708382, 59.924696 ): " << h
			<< endl << endl;
...
std::string placedefinition( const wdb::GdalLoadConfiguration &conf,
		const gdal::GdalFile &f, const boost::filesystem::path &file,
		wdb::load::LoaderDatabaseConnection &db )
{
	...
	try{
		placeName = db.getPlaceName( f.xNumber(), f.yNumber(), f.xIncrement(),
				f.yIncrement(), f.startX(), f.startY(), f.projDefinition() );
	...
	if( add ){
		if( !conf.loading().loadPlaceDefinition ){
			LOG->errorStream()
					<< "There is no placedefinition for this field in the database.\n"
					...
					<< "'" << boost::filesystem::basename( file )
					<< "' use --placename.\n";
			exit( 1 );
		}
		...
		LOG->infoStream() << "Trying to add placedefinition: '" << placeName
				<< "'.";
		try{
			placeName = db.addPlaceDefinition( placeName, f.xNumber(), f.yNumber(),
					f.xIncrement(), f.yIncrement(), f.startX(), f.startY(),
					f.projDefinition() );
...
int main( int argc, char ** argv )
{
	...
	try{
		...
		LOG->fatalStream() << "Cant connect to wdb. " << ex.what();
		exit( 1 );
	}
}
